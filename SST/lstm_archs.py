...
class standard_lstm(Model):
    ...
    def variational_inference(self,rec_input_seq,rec_pred):
        ...
        train_fields = np.load('train_fields.npy',allow_pickle=True).data.reshape(427,-1)

        mean_val, var_val = np.mean(train_fields), np.var(train_fields)

        # Load mean
        pod_mean = np.load('Snapshot_Mean.npy')

        # Remove mean and land points
        test_fields = test_fields[:,mask]
        test_fields = test_fields - pod_mean
